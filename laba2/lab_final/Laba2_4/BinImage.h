...
public:
    ...
    BinImage<T> operator!() {
        ...
        for (int i = 0; i < _height; ++i)
            for (int j = 0; j < _width; ++j) 
                if (_matrix[i][j] != 0) { val1 = _matrix[i][j]; break; }

        for (int i = 0; i < _height; ++i) {
            for (int j = 0; j < _width; ++j) {
                if (_matrix[i][j] == 0) { res._matrix[i][j] = val1; }
                else { res._matrix[i][j] = 0; }
   ...
   T& operator()(int x, int y) {
    ...
    void rdraw_rectangle(int x_1, int y_1, int x_2, int y_2, int i = 0, int j = 0, T value = 1) const  {
        if (i <= x_2 && i < _height) {
            if (j <= y_2 && j < _width) {
                _matrix[i][j] = value;
